...
$app->post('/stations', function () use ($app) {
    try {
        ...
        if(isset($_SESSION['user'])) {
            ...
            // Save station.
					...
					$attributes['create_date'] = $attributes['create_date'];
					$attributes['zone_id'] = $attributes['zone_id'];
					$attributes['screenid_type'] = $attributes['screenid_type'];
					$attributes['desktop_id'] = $attributes['desktop_id'];
					$attributes['fp_id'] = $attributes['fp_id'];
					$attributes['pp_id'] = $attributes['pp_id'];	
					$attributes['kbd_id'] = $attributes['kbd_id'];
					$attributes['mos_id'] = $attributes['mos_id'];
					$attributes['ups_id'] = $attributes['ups_id'];
					$attributes['cam_id'] = $attributes['cam_id'];
            ...
            // Update station.
				...
				$S_Id = $input["StationId"];
				$sqlQuery =	"UPDATE stations SET active = 0 WHERE id = $S_Id";		
...
$app->post('/stations/Update', function () use ($app) {
    try {
// Verify if the user is logged.
        if(isset($_SESSION['user'])) {
            // The user is logged. Get input.
            ...
            // Update station.
					$attributes = ApiUtils::mapToAttributes("StationUp", $input); 					
					...
					$attributes['cam_id'] = $attributes['cam_id'];
					$attributes['active'] = $attributes['active'];				
					$station = new StationUp($attributes);
					$station->save();
...
$app->post('/stations/search', function () use ($app) {
    try {
        // Verify if the user is logged.
        if(isset($_SESSION['user'])) {
            // The user is logged. Get input.
